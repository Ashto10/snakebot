...
function getSearchResults(searchTerm, allowNSFW) {
  const url = `https://www.google.com/search?q=${searchTerm}&tbm=isch&tbs=isz:m${
    allowNSFW ? '' : '&safe=active'
  }`
  ...
  return new Promise(async (resolve, reject) => {
    ...
    ;(imgWidth = sodaBottle.bitmap.width),
      ...
      (label = new jimp(imgWidth / 3, imgHeight, (error, img) => {
        if (error) {
          return reject({ errorType: INVALID_IMAGE, errorDetails: error })
        }
        return img
      }))
    ...
    const { error, firstHalf, secondHalf } = splitText(
      searchTerm,
      FONT_BLACK_18,
      FONT_BLACK_14,
      maxTextWidth,
